...
const app = {
    ...
    async gerarRelatorioRoteiro() {
        ...
        const veiculo = (typeof TurnoEngine !== 'undefined' && TurnoEngine.dados?.veiculo) || 'Carro';
        const veiculoSanitizado = veiculo.replace(/\s+/g, '_');
    ...
    _capturarGraficoVelocidade() {
        ...
        try { return chart.toBase64Image('image/png', 1.0); } catch (e) { return null; }
    },
    ...
    async gerarRelatorioRodagem() {
        ...
        if (graficoBase64) {
            ...
            doc.setFontSize(9); doc.setFont(undefined, 'bold'); doc.setTextColor(34, 211, 238);
    ...
    async gerarECompartilharLaudo(dadosRecuperados = null) {
        ...
        const analista = getVal(dadosRecuperados?.operador, 'laudo-ctx-operador', this.operadorAtual || 'N/A');
        ...
        const veiculo = getVal(dadosRecuperados?.veiculo, 'laudo-ctx-veiculo', (typeof TurnoEngine !== 'undefined' && TurnoEngine.dados?.veiculo) || 'N/A');
        ...
        // — Salva —
        // — Salva e Compartilha —
        const veiculoSanitizado = (veiculo || 'Carro').replace(/[^a-zA-Z0-9]/g, '_').replace(/_+/g, '_');
        const vinSanitizado = (vin || 'NAO_INFORMADO').replace(/[^a-zA-Z0-9]/g, '_').replace(/_+/g, '_');
        const analistaSanitizado = (analista || 'Analista').split(' ')[0].replace(/[^a-zA-Z0-9]/g, '_').replace(/_+/g, '_');
        ...
        if (!dadosRecuperados) {
            ...
            const r = {
                titulo: obsEl?.dataset.tituloIA || 'Ocorrência Manual',
                categoria: obsEl?.dataset.categoriaIA || 'Geral',
                severidade: obsEl?.dataset.severidadeIA || 'N/A',
                causaRaiz: obsEl?.dataset.causaRaizIA || '',
                parecerFinal: parecer
            };
            const totalMidias = this.fotos ? this.fotos.length : 0;
            try {
                if (typeof salvarIssueNoFirestore === 'function') {
                    await salvarIssueNoFirestore(r, obsEl?.value || parecer, { total: totalMidias, imagens: totalMidias, videos: 0 });
                }
            } catch (err) {
    ...
    async salvarECompartilharPDF(doc, fileName, textShare, titleShare = 'Relatório Ford') {
        ...
        try {
            ...
            try { doc.save(fileName); } catch (e) { /* ignore */ }
        }
    },
    ...
    async gerarRelatorioPerformanceGPS(viagem) {
        ...
        const modeloCarro = (v.veiculo && v.veiculo !== v.vin) ? v.veiculo : ((typeof TurnoEngine !== 'undefined' && TurnoEngine.dados?.veiculo) || 'Carro');
        const vinCarro = v.vin || (v.veiculo && v.veiculo.length > 5 ? v.veiculo : (this.vinGlobal || 'N/A'));
    ...
    rastreadorGpsID = navigator.geolocation.watchPosition((posicao) => {
        ...
        if (modoSelecionado === 'RODAGEM') {
            ...
            if (pts.length > 0) {
                ...
                pts.forEach(p => {
                    const v = p.vel || 0;
                    if (v < 80) f1++; else if (v < 160) f2++; else if (v < 230) f3++; else f4++;
                });
